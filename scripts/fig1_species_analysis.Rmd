...
#Load in species level bracken counts, which was pre-processed to exclude reads classified as contaminants
...
#more stats including median taxa normalized abun for AD/ctrl, IQR for AD/ctrl and Cliff's delta (AD vs ctrl)
#for info on Cliff's delta: https://garstats.wordpress.com/2016/05/02/robust-effect-sizes-for-2-independent-groups/
additional_stats_fn <- function(x){
  taxa_to_test <- taxa_names_after_filter
  
  results_list <-  vector(mode="list", length = length(taxa_to_test))
  ...
  for (i in 1: length(taxa_to_test)){
    ...
    #Get Cliff's delta
    #multiply by -1 so that effects that are more pronounced in AD are positive, while effects that are more pronounced in ctrl are negative for Cliff's delta
    cliff_d <- (cliff.delta(d=test_df$test_taxa, f=test_df$AD_hse)$estimate)*(-1)
    AD_median <- median(test_df%>%dplyr::filter(AD_hse=="AD_house")%>%pull(test_taxa))
    AD_IQR <- stats::IQR(test_df%>%dplyr::filter(AD_hse=="AD_house")%>%pull(test_taxa))
    ctrl_median <- median(test_df%>%dplyr::filter(AD_hse=="Healthy_house")%>%pull(test_taxa))
    ctrl_IQR <- stats::IQR(test_df%>%dplyr::filter(AD_hse=="Healthy_house")%>%pull(test_taxa))
    
    results_list[[i]] <- data.frame("species"=taxa,
                                    "cliff_delta"=cliff_d,
                                    "AD_median"=AD_median,
                                    "AD_IQR"=AD_IQR,
...
wilcox_and_bind_fn <- function(input){ #input can be AC_caregiver_count_CSS etc.
  ...
  colnames(output_wilcox_df)[1] <- "species"
  
  #stats results
  stats_df <- additional_stats_fn(input)
  stats_df <- do.call("rbind", stats_df)
  
  output_wilcox_df <- merge(output_wilcox_df, stats_df, by = "species")
  return(output_wilcox_df)
}

prepare_scatter_df_fn <- function(input_CSS, wilcox_df){
  taxa_to_keep <- wilcox_df %>% filter(padj < 0.05) %>% pull(species) %>% 
...
get_index_boxplot <- function(x, comparison, index){
  ...
  else if (comparison=="ethnicity"){
    df <- x %>% filter(child_disease_state=="1" & body_region %in%  c("VF","AC"))
    df$plotting_category <- paste0(df$plotting_category,"_",df$race)
    df$plotting_category <- factor(df$plotting_category,
    levels = c('AD_child_VF_Chinese', 'AD_child_VF_Malay',
               'AD_child_AC_Chinese', 'AD_child_AC_Malay', 
               'AD_caregiver_VF_Chinese', 'AD_caregiver_VF_Malay',
               'AD_caregiver_AC_Chinese', 'AD_caregiver_AC_Malay'), ordered = TRUE)
    my_comparisons <- list( c('AD_child_VF_Chinese', 'AD_child_VF_Malay'), 
                          c('AD_child_AC_Chinese', 'AD_child_AC_Malay'),
                          c('AD_caregiver_VF_Chinese', 'AD_caregiver_VF_Malay'),
                          c('AD_caregiver_AC_Chinese', 'AD_caregiver_AC_Malay'))
  }
  
  else if(comparison=="bathing"){ #exclude lesions
    df <- x %>% filter(sample_category=="child" & body_region %in%  c("VF","AC") )
    df$plotting_category <- paste0(df$plotting_category,"_",df$child_shower_cat_fmt)
    df$plotting_category <- factor(df$plotting_category,
    levels = c('healthy_child_VF_twice_per_day', 'healthy_child_VF_daily_or_less',
               'healthy_child_AC_twice_per_day', 'healthy_child_AC_daily_or_less',
               'AD_child_VF_twice_per_day', 'AD_child_VF_daily_or_less',
               'AD_child_AC_twice_per_day', 'AD_child_AC_daily_or_less'), ordered = TRUE)
    
      my_comparisons <- list(c('healthy_child_VF_twice_per_day', 'healthy_child_VF_daily_or_less'), 
                          c('healthy_child_AC_twice_per_day', 'healthy_child_AC_daily_or_less'),
                        c('AD_child_VF_twice_per_day', 'AD_child_VF_daily_or_less'),
                        c('AD_child_AC_twice_per_day', 'AD_child_AC_daily_or_less'))
    }
...
```{r filter_rare}
subjects_bracken_renorm_df <- data.frame(subjects_bracken_renorm_df)
rel_abun_col_filter <- vector(length=ncol(subjects_bracken_renorm_df)) 
rel_abun_col_filter[1] <- TRUE
for (i in 2:ncol(subjects_bracken_renorm_df)) {
  rel_abun_col_filter[i] <- ifelse(median(subjects_bracken_renorm_df[,i]) >= 0.001, TRUE, FALSE )
}
...
Perform wilcoxon rank sum tests on the datasets, for these species after filtering. Collect statistics for main manuscript supplementary table 2.

Only keep those with padj < 0.05 in Supplementary table 2

```{r, message=FALSE}
########Testing children samples#####

child_AC_CSS_wilcox_df <- wilcox_and_bind_fn(AC_child_count_CSS) %>% dplyr::filter(padj<0.05)

child_VF_CSS_wilcox_df <- wilcox_and_bind_fn(VF_child_count_CSS) %>% dplyr::filter(padj<0.05)

write_tsv(child_AC_CSS_wilcox_df, file="../data/processed/tableS2_child_AC_CSS_wilcox.tsv")
write_tsv(child_VF_CSS_wilcox_df, file="../data/processed/tableS2_child_VF_CSS_wilcox.tsv")


########Testing caregiver samples#####
#As can be seen, nothing passes p adj < 0.05
...
AC_child_taxa_scatter <- ggplot(AC_child_CSS_scatter_input, aes(x=median_CSS_counts_healthy, y=median_CSS_counts_AD)) + geom_point() + 
  ...
  geom_point(data=AC_child_CSS_scatter_input %>% 
             ...
             colour="red") + geom_line(data=data.frame(x=c(5,13),y=c(5,13)), aes(x=x, y=y))

AC_child_taxa_scatter 

ggsave("../plots/fig1a_AC_child_scatter.svg", plot=AC_child_taxa_scatter, width=10, height=10)
...
VF_child_taxa_scatter <- ggplot(VF_child_CSS_scatter_input, aes(x=median_CSS_counts_healthy, y=median_CSS_counts_AD)) + geom_text_repel(aes(label=species)) + geom_point()+
    theme_classic() + xlim(5, 10) + ylim(5, 10) + geom_line(data=data.frame(x=c(5,10),y=c(5,10)), aes(x=x, y=y))

VF_child_taxa_scatter 

ggsave("../plots/fig1a_VF_child_scatter.svg", plot=VF_child_taxa_scatter, width=10, height=10)
...
Shannon diversity comparison between individuals of different ethnicities from AD houses.

```{r echo=FALSE}

AD_ethnicity_shannon_boxplot <- get_index_boxplot(subjects_bracken_renorm_shannon,
                                                            comparison = "ethnicity",
                                                            index="shannon") 

AD_ethnicity_shannon_boxplot


```

Shannon diversity for children 
...
child_bracken_renorm_shannon <- subjects_bracken_renorm_shannon %>% filter(sample_category=="child")

child_bracken_renorm_shannon <- merge(child_bracken_renorm_shannon,
                                      demographics %>% dplyr::select(subject_ID, child_shower_cat),
                           by="subject_ID")

child_bracken_renorm_shannon$child_shower_cat_fmt <- ifelse(child_bracken_renorm_shannon$child_shower_cat==3,
                                                            "twice_per_day","daily_or_less") 

child_bathing_shannon_boxplot <- get_index_boxplot(child_bracken_renorm_shannon,
                                                            comparison = "bathing",
                                                            index="shannon") 

child_bathing_shannon_boxplot

length(which(child_bracken_renorm_shannon$body_region=="VF" & 
        child_bracken_renorm_shannon$child_disease_state==0 & 
        child_bracken_renorm_shannon$child_shower_cat_fmt=="twice_per_day")) #17

length(which(child_bracken_renorm_shannon$body_region=="VF" & 
        child_bracken_renorm_shannon$child_disease_state==0 & 
        child_bracken_renorm_shannon$child_shower_cat_fmt=="daily_or_less")) #13



length(which(child_bracken_renorm_shannon$body_region=="AC" & 
        child_bracken_renorm_shannon$child_disease_state==0 & 
        child_bracken_renorm_shannon$child_shower_cat_fmt=="twice_per_day")) #17
...
length(which(child_bracken_renorm_shannon$body_region=="VF" & 
        child_bracken_renorm_shannon$child_disease_state==1 & 
        child_bracken_renorm_shannon$child_shower_cat_fmt=="twice_per_day")) #19

length(which(child_bracken_renorm_shannon$body_region=="VF" & 
        child_bracken_renorm_shannon$child_disease_state==1 & 
        child_bracken_renorm_shannon$child_shower_cat_fmt=="daily_or_less")) #7

length(which(child_bracken_renorm_shannon$body_region=="AC" & 
        child_bracken_renorm_shannon$child_disease_state==1 & 
        child_bracken_renorm_shannon$child_shower_cat_fmt=="twice_per_day")) #20
...
Are there any significant differences in S. aureus normalized relative abundances with showering habits?

```{r}

bathing_boxplot_fn <- function(df=VF_child_count_CSS, species, title){
  species <- enquo(species)
  df$LibraryID <- row.names(df)
  df <- merge(df, child_bracken_renorm_shannon %>% 
                dplyr::select(LibraryID,child_shower_cat_fmt),by="LibraryID")
  df$plotting_cat <- paste0(df$AD_hse,"_",df$child_shower_cat_fmt)
  
  df$plotting_cat <- factor(df$plotting_cat,
    levels = c('Healthy_house_twice_per_day', 'Healthy_house_daily_or_less',
               'AD_house_twice_per_day', 'AD_house_daily_or_less'), ordered = TRUE)
    
      my_comparisons <- list(c('Healthy_house_twice_per_day', 'Healthy_house_daily_or_less'), 
                          c('AD_house_twice_per_day', 'AD_house_daily_or_less'))
      
    plot <- ggplot(df, aes(x=plotting_cat, y=!!species)) + geom_boxplot() + theme_classic() + 
  stat_compare_means(paired = FALSE, method = "wilcox.test", comparisons=my_comparisons) +
    theme(axis.title.x=element_blank(), axis.text.x = element_text(angle = 45, hjust=1))
  return(plot)
...
bathing_boxplot_fn(df=VF_child_count_CSS, species=Staphylococcus_aureus)
