...
VF_all_rfe <- rfe(x=VF_all_count_CSS[,-42], y=VF_all_count_CSS$AD_hse,
                 sizes = subsets,
...
##Just call glm on just these selected predictors.
...
confusionMatrix(VF_all_CSS_post_rfe_model) #0.7876 average accuracy 

#Look at summary(VF_all_CSS_post_rfe_model) for a list of significant taxa

summary(VF_all_CSS_post_rfe_model)

#input_model is an object of class "train"
tidy_lm_res <- function(input_model){
  res_df <- data.frame(summary(VF_all_CSS_post_rfe_model)$coefficients)
  res_df$feature <- row.names(res_df)
  colnames(res_df) <- c("Estimate", "Std_err", "z_value", "p_value", "feature")
  
  res_df <- res_df %>% dplyr::select(feature, Estimate, Std_err, z_value, p_value)
  res_df$padj <- p.adjust(res_df$p_value, method="fdr")
  return(res_df)
}

##This is part of supplementary table 3 in the main manuscript 
VF_all_CSS_post_rfe_model_res <- tidy_lm_res(VF_all_CSS_post_rfe_model)

write_tsv(VF_all_CSS_post_rfe_model_res, file="../data/processed/VF_CSS_rfe_lm_results.tsv")
...
AC_all_rfe <- rfe(x=AC_all_count_CSS[,-42], y=AC_all_count_CSS$AD_hse,
                 sizes = subsets,
                 rfeControl = rfe_ctrl, metric = "Accuracy")
...
##Just call glm on just these 15 predictors.
...
print(AC_all_CSS_post_rfe_model) # ROC 0.87   Sens 0.8111111      Spec  0.7622222   
     
confusionMatrix(AC_all_CSS_post_rfe_model) #0.7895 average accuracy
...
#This is part of supplementary table 3 in the main manuscript 
AC_all_CSS_post_rfe_model_res <- tidy_lm_res(AC_all_CSS_post_rfe_model)

write_tsv(AC_all_CSS_post_rfe_model_res, file="../data/processed/AC_CSS_rfe_lm_results.tsv")
...
print(VF_bal_1denom) #ROC 0.7671296  Sens 0.7444444  Spec 0.6811111
...
print(VF_bal_2denom)
varImp(VF_bal_2denom) #The two most important variables are X65 (100) and X333 (98)

attr(VF_bal_input[[2]][,"X65"], "denominator")  #Refers to "Actinomyces_oris" and "Staphylococcus_hominis"
attr(VF_bal_input[[2]][,"X333"], "denominator")  #Refers to "Microbacterium_aurum" and "Staphylococcus_hominis"

##model with only these two taxa
#"Actinomyces_oris" and "Staphylococcus_hominis"
set.seed(123)
VF_bal_Aoris_Shom <- train(AD_hse~X65 , data=VF_bal_input[[2]], trControl=train_control, method="glm", family=binomial())
...
print(AC_bal_1denom) #ROC 0.8309259  Sens 0.7722222  Spec 0.7366667
...
print(AC_bal_2denom)  #ROC 0.8209259  Sens 0.7555556  Spec 0.77
varImp(AC_bal_2denom) #The two most important variables are X711 (100) and X65 (93.25)

attr(AC_bal_input[[2]][,"X711"], "denominator")  #Refers to "Streptococcus_sanguinis" and "Staphylococcus_hominis"
attr(AC_bal_input[[2]][,"X65"], "denominator")  #Refers to "Actinomyces_oris" and "Staphylococcus_hominis"

##model with only these two taxa
set.seed(123)
AC_bal_Ssang_Shom <- train(AD_hse~X711 , data=AC_bal_input[[2]], trControl=train_control, method="glm", family=binomial())
